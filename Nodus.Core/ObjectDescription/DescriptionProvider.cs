...
public class DescriptionProvider : ReactiveObject, ICustomTypeDescriptor
{
    /// <summary>
    /// The properties
    /// </summary>
    public readonly List<ValueDescriptor> Descriptors = new();

    /// <summary>
    /// Adds the property.
    /// </summary>
    /// <param name="property">The property.</param>
    public void AddDescriptor(ValueDescriptor descriptor)
    {
...
internal class ScriptableObjectPropertyDescriptor : PropertyDescriptor
{
    ...
    public ScriptableObjectPropertyDescriptor(ValueDescriptor target) :
    ...
    public override Type ComponentType => typeof(DescriptionProvider);

    public override bool IsReadOnly
    {
        get
        {
            var attr = this.Attributes[typeof(ReadOnlyAttribute)] as ReadOnlyAttribute;
    ...
    {
        if (component is DescriptionProvider options)
        {
            var obj = options.Descriptors.Find(x => x.Name == this.Name);
