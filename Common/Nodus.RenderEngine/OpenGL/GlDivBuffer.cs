...
/// </summary>
public unsafe interface IGlDivBuffer : IUnmanagedHook
{
    /// <summary>
    ...
    void UpdateData(nuint offset, nuint size, void* data);
}
...
public class GlDivBuffer : GlObject, IGlDivBuffer
{
    ...
    public GlDivBuffer(GL gl, BufferTargetARB bufferType, nuint allocSize, bool autoUnbind = true) : base(gl)
    {
    ...
    public unsafe void Allocate(nuint size)
    {
        ...
        Context.TryThrowNextError($"Failed to allocate buffer data. Type={bufferType}, Size={size}");
    }

    public unsafe void UpdateData(nuint offset, nuint size, void* data)
    {
        Context.BufferSubData(bufferType, (IntPtr) offset, size, data);
        Context.TryThrowNextError($"Failed to update buffer data. Type={bufferType}, Size={size}");
    }
